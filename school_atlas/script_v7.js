...
function generate_and_download_xlsx() {
  var raw_data = [];
  ...
  for (const PLZ in table_data) {
    ...
    for (var school_id in table_data[PLZ]) {
      if (Object.prototype.hasOwnProperty.call(table_data[PLZ], school_id)) {
        ...
        var info_data = [""].concat(current_school["info_data"]);
        // array with an empty cell, followed by id, address and description of the current_school
  ...
  XLSX.utils.book_append_sheet(
    ...
    "Interpreted data 2022-2023"
  );

  // write the temporary file and download it
  XLSX.writeFile(workbook, "data_2022_2023.xlsx");
...
function complete_analysis_and_generate_xlsx() {
  ...
  if (urlParams.has("download_xlsx")) {
    generate_and_download_xlsx();
  }
}
...
(async () => {
  ...
  // 2nd class: 18.912
  // 3rd class: 18.699
  // 4th class: 17.866 (this is the small difference mentionebyd above)
  ...
  for (const PLZ in elementary_schools_by_PLZ) {
    for (var i = 0; i < elementary_schools_by_PLZ[PLZ].length; i++) {
      ...
      table_data[PLZ][elementary_schools_by_PLZ[PLZ][i].id]["info_data"] = [
        ...
        elementary_schools_by_PLZ[PLZ][i].address,
        elementary_schools_by_PLZ[PLZ][i].description,
      ];
